...
class DataManager:
    ...
    Methods:
        ...
        get_FM_historic_transition: Retrieves the managed forest historic transition from the database.
        get_FM_historic_disturbances: Retrieves the managed forest historic disturbances from the database.
    ...
    def get_FM_historic_transition(self):
        """
        Retrieves the managed forest historic transition from the database.

        Returns:
            pandas.DataFrame: A DataFrame containing the managed forest historic transition.
        """
        table = "FM_HISTORIC_transition_2100"
        dataframe = pd.read_sql(
            "SELECT * FROM '%s'" % (table),
            self.engine,
        )
    ...
    def get_FM_historic_disturbances(self):
        """
        Retrieves the managed forest historic disturbances from the database.

        Returns:
            pandas.DataFrame: A DataFrame containing the managed forest historic disturbances.
        """
        table = "FM_HISTORIC_disturbances_2100"
        dataframe = pd.read_sql(
            "SELECT * FROM '%s'" % (table),
            self.engine,
        )
    ...
    def get_FM_disturbance_events(self):
        """
        ...
        """
        table = "FM_disturbances_2100"
        query = f"SELECT * FROM '{table}'"
        dataframe = pd.read_sql(query, self.engine)

        return dataframe

    def get_AF_disturbance_events(self):
        """
