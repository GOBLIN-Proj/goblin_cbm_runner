...
class Harvest:
    ...
    def get_harvest_areas(self):
        ...
        yield_proportions_dict = {
            list(yield_proportions_list[i].keys())[0]: list(yield_proportions_list[i].values())[0]
        ...
        for year in forest_harvest.index:
            for yc in yield_dict.keys():
                ...
                harvest_area.at[count, "mineral"] = self.calculate_area(proportion, volumn, yield_volumns) * soils_proprotions.at[year, "mineral"]
                harvest_area.at[count, "peat"] = self.calculate_area(proportion, volumn, yield_volumns) * soils_proprotions.at[year, "organic"]
    ...
    def generate_soil_proportions(self):
        ...
        soils_proportion["mineral"] = forest_inventory["mineral_kha"]/forest_inventory["total_kha"]
        soils_proportion["organic"] = forest_inventory["organic_kha"]/forest_inventory["total_kha"]
    ...
    def make_yields(self):
        ...
        harveset_dict = self.data_manager_class.forest_baseline_disturbance_dict

        yield_names = self.data_manager_class.yield_name_dict

        volumes = {} 

        for yield_name in yield_names["CF"].keys():

            mask = (yield_table.index == yield_names["CF"][yield_name])

            col = harveset_dict["DISTID1"]["CF"][yield_names["CF"][yield_name]]
