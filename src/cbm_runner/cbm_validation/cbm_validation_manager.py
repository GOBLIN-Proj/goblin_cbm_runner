...
from cbm_runner.cbm_validation.validation_data import get_local_dir

class CBM_ValidationManager:
    """
    This class is responsible for retrieving the validation data generated when running scenarios.

    To generate validation data, the Runner attribute must be set to ```gen_validation``` True and the valdiation path must be set.
    ...
    def save_baseline_forest(self, baseline_forest):
        """
        Saves the baseline forest data to a CSV file.

        Args:
            baseline_forest: The baseline forest data.
        """
        ValidationData.gen_baseline_forest(self.path, baseline_forest)

    def gen_validation_data(self, years, scenario, data):
        """
        Generates validation data for a scenario.
        ...
        ValidationData.gen_sit_events(self.path, data, scenario)
        ValidationData.gen_disturbance_statistics(self.path, data, years, scenario)
        ValidationData.merge_events(self.path, scenario)
