...
class Disturbances:
    ...
    def _handle_legacy_forest(self, row_data, context, dataframes):
        ...
        harvest_proportion = context["harvest_proportion"]
        ...
        if dist == "DISTID3":
            ...
            try:
                if disturbance_dataframe.loc[mask, "Amount"].item() == "?":
                    row_data["Amount"] = harvest_proportion 
                else:
                    row_data["Amount"] = disturbance_dataframe.loc[
                        mask, "Amount"
                    ].item()
