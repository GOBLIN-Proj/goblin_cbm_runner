...
from cbm_runner.forest_sim.validation_data import get_local_dir

class ForSimValidationManager:
    """
    This class is responsible for retrieving the validation data generated when running scenarios.

    To generate validation data, the ForestSimRunner attribute must be set to ```gen_validation``` True and the valdiation path must be set.
